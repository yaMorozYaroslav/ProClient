...
export const updateItem = async(req, res)=> {
	...
	const {id} = req.params
	const {title, description, creator, photo, price, condition, createdAt} = req.body

	if(!mongoose.Types.ObjectId.isValid(id)) 
		return res.status(404).send(`No item with id: ${id}`)

	const updatedItem = {creator, title, description, photo, condition, price, _id: id, createdAt}
