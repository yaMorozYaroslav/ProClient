...
export const ItemExcerpt =({item, setCurrentId, single, setSingle,
     ...
     const noImage = 'https://sotown.com/manual/images/7/7c/Noimage.png'
	...
	const handDelete =(e)=> {
		e.preventDefault()
		removeItem(item._id)
		setTimeout(() => alert('Element has been deleted.'),1000)
		}
   ...
   const listButt = {cursor: 'pointer', padding: '4px', fontSize: '20px'}
	...
	<article style={{border:`6px solid #${randomColor}`, textAlign:'center',
	  ...
	  <img src={!showPointer?(item.photo||noImage):pointer} alt='' 
	  ...
	  {(userData.user && 
	   ...
	   (<>
	  <button style={button} onClick={handDelete}>Delete</button>
